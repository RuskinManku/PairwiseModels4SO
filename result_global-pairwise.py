...
from Global_BERTPair import GlobalInformationModule,pair_wise_model
#from Global_ALBERTPair import GlobalInformationModule,pair_wise_model
...
def main():
    ...
    global_test_model=GlobalInformationModule()
    global_test_model=global_test_model.to(device)

    pair_model=pair_wise_model()
    pair_model=pair_model.to(device)

    #-----------------------Upload corresponding checkpoint
    checkpoint=torch.load("global-bertpair_sind_model_epoch_3_checkpoint_20078.tar")
    global_test_model.load_state_dict(checkpoint["global_model_state_dict"])
    pair_model.load_state_dict(checkpoint["pair_model_state_dict"])

    global_test_model.eval()
    pair_model.eval()
    
    print(global_test_model)
    print(pair_model)
    ...
    for i in range(test_paragraphs.shape[0]):
        ...
        with torch.no_grad():
            pair_embeddings,data_after_level_two,batch_index_to_pair_dict = global_test_model(test_paragraph_model)
