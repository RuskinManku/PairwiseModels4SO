...
class pair_wise_model(nn.Module):
    def __init__(self,d_model=768):
        ...
        self.encode_pair_albert=PairEncoder(model_name='ALBERT')
        self.encode_pair_bert=PairEncoder(model_name='BERT')
        self.dropout = nn.Dropout(0.1)
        self.pair_weight_albert=nn.Linear(d_model,2)
        self.pair_weight_bert=nn.Linear(d_model,2)

    def forward(self, src_paragraphs):
        pooled_output_albert,batch_index_to_pair_dict=self.encode_pair_albert(src_paragraphs)
        pooled_output_bert,_=self.encode_pair_bert(src_paragraphs)
        pooled_output_albert=self.dropout(pooled_output_albert)
        pooled_output_bert=self.dropout(pooled_output_bert)
        output_logits=[]
        for i in range(pooled_output_albert.size()[0]):
            classifier_output=self.pair_weight_albert(pooled_output_albert[i]) + self.pair_weight_bert(pooled_output_bert[i])
...
def main():
    ...
    if args.dataset=="sind":
        ...
        MODEL_SAVE_NAME="ensemblepair_sind_model"
    ...
    CHECKPOINT=None#torch.load("ensemblepair_sind_model_epoch_5_checkpoint_20078.tar", map_location='cpu')
    if CHECKPOINT:
