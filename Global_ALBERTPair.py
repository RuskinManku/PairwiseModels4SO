...
class GlobalInformationModule(nn.Module):
    def __init__(self):
        ...
        encoder_layers = TransformerEncoderLayer(d_model=768, nhead=12, dim_feedforward=3072, dropout=0.1)
    ...
    def forward(self,src_paragraphs):
        ...
        #transformer layer
        data_after_level_two=data_after_level_two.transpose(1,0)
        out_transformer=self.transformer_encoder(data_after_level_two)
        out_transformer=out_transformer.transpose(1,0)

        # print(data_after_level_two.size())
        # print(pair_embeddings.size())
        return pair_embeddings,out_transformer,batch_index_to_pair_dict
...
class pair_wise_model(nn.Module):
    def __init__(self,d_model=768):
        ...
        self.W_2=nn.Linear(d_model,2)
        #Weight for the pairs
        self.dropout = nn.Dropout(0.1)
        self.pair_weight=nn.Linear(d_model,2)

    def forward(self, pair_embeddings,data_after_level_two,batch_index_to_pair_dict):
        pooled_output = torch.tanh(self.dense(pair_embeddings))   #Replicating pooler like in huggingface bert [1] output
        ...
        for i in range(pooled_output.size()[0]):
            ...
            classifier_output=self.pair_weight(pooled_output[i]) \
                              +self.W_1(data_after_level_two[idx][j]) \
                              +self.W_2(data_after_level_two[idx][k])
...
def main():
    ...
    if args.dataset=="sind":
        ...
        MODEL_SAVE_NAME="global-albertpair_sind_model"
    ...
    params = list(global_model.parameters()) + list(pair_model.parameters())
    optimizer_1 = optim.Adam(params, lr=5e-6)
    scheduler_1 = torch.optim.lr_scheduler.StepLR(optimizer_1, step_size=1, gamma=0.1)

    CHECKPOINT=None#torch.load("global-albertpair_sind_model_epoch_7_checkpoint_20078.tar", map_location='cpu')
    if CHECKPOINT:
    ...
    if RUN_INITIAL_VALIDATION:
        with torch.no_grad():
            ...
            for j in range(len(val_x_batches)):
                ...
                pair_embeddings,data_after_level_two,batch_index_to_pair_dict = global_model(inputs)
                output_logits = pair_model(pair_embeddings,data_after_level_two,batch_index_to_pair_dict)
